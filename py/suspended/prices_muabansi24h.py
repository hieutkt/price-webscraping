...
# Parameters
SITE_NAME = "muabansi24h"
BASE_URL = "https://muabansi24h.com/"
...
def get_category_list(top_html):
    ...
    categories = toppage_soup.find('ul',{'class':'dropdown-menua'})
    ...
    for cat in categories_tag:
        page = {}
        link = re.sub(".+muabansi24h\.com/", "", cat['href'])
...
def scrap_data(cat):
    ...
    cat_div = cat_soup.findAll('div', {'class' : 'col-md-3 item_mar'})
    for item in cat_div:
        row = {}
        good_name = item.find('h4')
        row['good_name'] = good_name.text if good_name else None
        price = item.find('span', {"class": "price_new"})
        row['price'] = price.text if price else None
        old_price = item.find('span', {"class": "price_old"})
...
def find_next_page(cat):
    ...
    if pagination:
        ...
        next_button = None
    if next_button:
        link = re.sub(".+muabansi24h\.com", "", next_button['href'])
