...
def daily_task():
    ...
    menu_list = soup.find('ul', class_='menulink').find_all('div', class_='title')
    for item in menu_list:
        href = item.find('a').get('href')
        title = item.find('a').text.strip()
        if href not in urls and href != '#':
            urls.append(href)
            titles.append(title)
    # print(len(urls))
    # print(urls)
    ...
    while j < len(urls):
        ...
        while pagination:
            if i != 0:
                ...
                try:
                    soup = BeautifulSoup(browser.page_source, 'lxml')
                    list = soup.find('div', class_='productlist-page').find_all('div', class_='product-col')
                except:
                    pagination = False
            if i == 0:
                try:
            ...
            for item in list:
                ...
                try:
                    good_name = item.find('h3', class_='product-name').text.strip()
                except:
                    good_name = None

                try:
                    id_ = item.find('h3', class_='product-name').find('a').get('href').strip()
                except:
                    id_ = None

                try:
                    price = item.find('div', class_='new').text.strip()
                ...
                try:
                    old_price = item.find('div', class_='old').text.strip()
