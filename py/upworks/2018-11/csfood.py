...
def daily_task():
    ...
    menu_list = soup.find_all('ul', class_='sub-menu')
    for item in menu_list:
        menu_list2 = item.find_all('a')
    ...
    while j < len(urls):
        ...
        while pagination:
            if i != 0:
                try:
                    wait.until(lambda browser: browser.find_element_by_css_selector('div.nav-links'))
                    elements = browser.find_elements_by_css_selector('div.nav-links .page-numbers')
                    if len(elements) == 1:
                        break
                    c=0
                ...
                try:
                    soup = BeautifulSoup(browser.page_source, 'lxml')
                    list = soup.find('ul', class_='product-list').find_all('li', class_='product-item')
                except:
                    pagination = False
            if i == 0:
                try:
            ...
            for item in list:
                ...
                try:
                    good_name = item.find('h5', class_='product-name').text.strip()
                except:
                    good_name = None

                try:
                    id_ = item.find('h5', class_='product-name').find('a').get('href').strip()
                except:
                    id_ = None

                try:
                    price = item.find('ins').find('span', class_='woocommerce-Price-amount').text.strip()
                ...
                try:
                    old_price = item.find('del').find('span', class_='woocommerce-Price-amount').text.strip()
