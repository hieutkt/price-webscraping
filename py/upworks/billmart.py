...
def daily_task():
    ...
    menu_list = soup.find_all('div', class_='submenu_cata')
    ...
    while j < len(urls):
        ...
        while pagination:
            if i != 0:
                ...
                try:
                    soup = BeautifulSoup(browser.page_source, 'lxml')
                    list = soup.find('div', id='product').find_all('article', class_='box-product')
                except:
                    pagination = False
            if i == 0:
                try:
            ...
            for item in list:
                ...
                try:
                    good_name = item.find('div', class_='product_title').text.strip()
                except:
                    good_name = None

                try:
                    id_ = BASE_URL + item.find('div', class_='product_title').find('a').get('href').strip()
                except:
                    id_ = None
                ...
                try:
                    old_price = item.find('div', class_='old-price').text.strip()
