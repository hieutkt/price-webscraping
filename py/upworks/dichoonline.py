...
SITE_NAME = "dichoonline"
BASE_URL = "http://dichoonline.com.vn/"
...
def daily_task():
    ...
    menu_list = soup.find_all('a', class_='product_km_hot')
    ...
    while j < len(urls):
        ...
        while pagination:
            if i != 0:
                ...
                try:
                    soup = BeautifulSoup(browser.page_source, 'lxml')
                    list = soup.find('div', class_='wrap-list-pro').find_all('div', class_='product-item')
                except:
                    pagination = False
            if i == 0:
                try:
            ...
            for item in list:
                ...
                try:
                    good_name = item.find('a', class_='product-name').text.strip()
                except:
                    good_name = None

                try:
                    id_ = BASE_URL + item.find('a', class_='product-name').get('href').strip()
                except:
                    id_ = None
