...
def daily_task():
    ...
    # Compress data
    zipcommand = "cd " + path_csv + "&& tar -cvzf " + site_name + date + \
        ".tar.gz *" + site_name + "_" + date + "* --remove-files"
...
def get_category_list(top_html):
    ...
    categories = toppage_soup.find("ul", {'class': 'fs-mnsul3'})
    ...
    categories_link = [re.sub(".+fptshop\.com\.vn/", "", i['href'])
...
def scrap_data(cat_name):
    ...
    cat_div = cat_soup.find('div', {'class': 'owl-item'})
    if cat_div:
        cat_div = cat_soup.findAll("div", {"class": "item"})
    else: cat_div = []
    data = []
    for item in cat_div:
        row = {}
