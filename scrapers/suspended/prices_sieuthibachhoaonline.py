...
# Parameters
SITE_NAME = "sieuthibachhoaonline"
BASE_URL = "http://sieuthibachhoaonline.com/"
...
def get_category_list(top_html):
    ...
    categories = toppage_soup.find('div',{'class':'flex-row container'})
    ...
    for cat in categories_tag:
        page = {}
        link = re.sub(".+sieuthibachhoaonline\.com/", "", cat['href'])
...
def scrap_data(cat):
    ...
    cat_div = cat_soup.findAll('div', {'class' : 'col-inner'})
    cat_div = [div for div in cat_div if not div.has_attr('id')]
    for item in cat_div:
        row = {}
        good_name = item.find('p', {'class': 'name product-title'})
        ...
        id1 = good_name.find("a") if good_name else None
...
def find_next_page(cat):
    ...
    cat_file = open(PATH_HTML + "cat_" + cat['name'] + "_" +
                    DATE + ".html").read()
    cat_soup = BeautifulSoup(cat_file, "lxml")
    next_button = cat_soup.find('a', {'class': 'next page-number'})
    if next_button:
        link = re.sub(".+sieuthibachhoaonline\.com/", "", next_button['href'])
